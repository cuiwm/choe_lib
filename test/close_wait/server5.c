...
#define MAX_EVENTS  32
    ...
    while (1) {
        int n, i;
        printf("while[%d]before epoll_wait\n", gi++);
        n = epoll_wait(epfd, events, MAX_EVENTS, -1);
        printf("while[%d]after epoll_wait: n:%d\n", gi++, n);

        for (i = 0; i < n; ++i) {

            printf("\nfor[%d]: epoll_wait return sock:%d <<<event:0x%x>>\n", gi++,  events[i].data.fd, events[i].events);

            if ( (events[i].events & EPOLLERR) ||
                    (events[i].events & EPOLLHUP) ||
                    (!(events[i].events & EPOLLIN)) )
            {
				uint32_t  ev_code = events[i].events;
                fprintf(stderr, "epoll events:%x<%s>", ev_code, EP_STR);

				if (events[i].events & EPOLLIN) {
					ev_code &= ~EPOLLIN;
					fprintf(stderr, "~EPOLLIN(%x)~", ev_code); 
				} 

                if  (events[i].events & EPOLLERR) {
					ev_code &= ~EPOLLERR;
                    fprintf(stderr, "~EPOLLERR(%x)~", ev_code);

                }

                if (events[i].events & EPOLLHUP)  {
					ev_code &= ~EPOLLHUP;
					fprintf(stderr, "~EPOLLHUP(%x)~", ev_code);
                }

                if (events[i].events & EPOLLRDHUP)  {
					ev_code &= ~EPOLLRDHUP;
                    fprintf(stderr, "~EPOLLRDHUP(%x)~", ev_code);
                }
				...
				printf("<<<<end1 listen sock:%u\n", listenfd);
                continue;
				printf("<<<<end2 listen sock:%u\n\n", listenfd);
            }
            else
            {
				...
				printf("\n>>>>>>begin handling conn sock:%u\n. will sleep 10s", events[i].data.fd);
                sleep(10);
...
events[i].data.fd);
                    ...
                    //jint s = write(events[i].data.fd, buf, count);
                ...
                if (done) {
                    printf("Not closed conn sock:%d\n", events[i].data.fd);
                    /*Closing the descriptor will make epoll remove it 
                     * from the set of descriptors which are monitored*/
                    //close(events[i].data.fd);
                }

				printf("<<<<<end handling conn sock:%u\n\n", events[i].data.fd);
